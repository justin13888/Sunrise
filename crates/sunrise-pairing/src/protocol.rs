...
/// Regenerate the Apple clients' `PairingFixture`.
///
/// `apps/apple/SunriseTests/PairingFixture.swift` holds one real, canonically
/// encoded [`PairingPayload`] as base64, because `SunriseCore.open` *decodes*
/// what it is given and Swift has no constructor for the type — a payload is
/// assembled out of a pairing, and a unit test has no second device. A stale
/// fixture fails loudly on the Swift side rather than quietly somewhere else,
/// which is what happened when field 2 was burned.
///
/// Ignored because it asserts nothing; it is a generator with a test's
/// signature so it can reach this crate's internals. Run it and paste the
/// output:
///
/// ```text
/// cargo test -p sunrise-pairing -- --ignored --nocapture apple_fixture
/// ```
#[cfg(test)]
#[test]
...
fn apple_fixture() {
    ...
    let signing = IdentitySigningKeyPair::from_secret_bytes(&[0x11; 32]);
    let id_s_pub = signing.public_bytes();
    let d_s = DeviceSigningKeyPair::from_secret_bytes(&[0x33; 32]);
    let d_d = DeviceDhKeyPair::from_secret_bytes([0x44; 32]);
    let body = DeviceCertInner {
        v: 1,
        device_id: device_id_from_pub(&d_s.public_bytes()),
        d_s_pub: d_s.public_bytes(),
    ...
    let mut stream_keys = BTreeMap::new();
    stream_keys.insert([0u8; 16], BTreeMap::from([(1u32, [0xCDu8; 32])]));
    ...
    let encoded = crate::payload::encode_pairing_payload(&payload).unwrap();
    println!("---- PairingFixture.base64 ----");
    for chunk in STANDARD.encode(&encoded).as_bytes().chunks(72) {
