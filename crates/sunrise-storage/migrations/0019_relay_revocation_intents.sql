-- 0019: the queue of "tell the relay this device is gone".
--
...
-- own `devices` row, set through `DELETE /api/v1/devices/{device_id}`.
--
...
--
-- One row per device, inserted by the revocation and deleted only when the
-- relay has confirmed it. `attempts` and `last_attempt_ms` exist so a relay
-- that is refusing the call is visible rather than silently retried forever.
