...
//!
//! [`crate::capture::parse`] can set a Task's stream, contexts, priority,
//! schedule, due date and estimate in one line, but only at creation. This is
//! the other half: the same sigils, applied to a Task that already exists, so
//! every field [`TaskPatch`] carries is reachable without a form.
//!
//! | Token | Effect |
//! |---|---|
...
//!   a title. An annotate line is not: silently appending "tomorow" to a
//!   task's name because the date failed to parse would be the opposite of
//!   helpful. Editing a title is its own operation.
//! * **Clearing is expressible.** [`TaskPatch`] distinguishes "leave alone"
//!   (`None`) from "set to nothing" (`Some(None)`), and without a `-` form the
//!   second is unreachable — a priority set by accident could never be
//!   removed.
//!
//! Like [`crate::capture::parse`] this is pure over its inputs and resolves
//! `#`/`@` against a caller-supplied candidate list, so it can run on every
//! keystroke for a live preview without touching a database. Callers pass
...
impl TaskEdit {
    ...
    pub fn preview(
        ...
        let name = |id: EntityRef, rows: &[NamedRef<'_>]| {
            rows.iter()
                .find(|c| c.id == id)
                .map_or_else(|| id.to_str(), |c| c.name.to_string())
        };
        ...
        if let Some(id) = self.stream {
            let n = streams
                .iter()
                .find(|s| s.id == id)
                .map_or_else(|| id.to_str(), |s| s.name.to_string());
            parts.push(format!("→ #{n}"));
        }
        for c in &self.add_contexts {
...
///
/// `streams` and `contexts` are the **live** candidate sets — archived rows
/// must be filtered out by the caller, exactly as they are for
/// [`crate::capture::parse`]: an archived entity stays on the tasks that carry
/// it, but must not be a target for new input.
#[must_use]
pub fn parse(
    input: &str,
    streams: &[NamedRef<'_>],
...
fn parse_context_token(edit: &mut TaskEdit, body: &str, contexts: &[NamedRef<'_>]) {
    match body.strip_prefix('-') {
        Some("") => edit.clear_contexts = true,
        Some(name) => match resolve(name, contexts) {
            Some(id) => edit.remove_contexts.push(id),
            None => edit.errors.push(EditError::UnknownContext(name.into())),
        },
        None => match resolve(body, contexts) {
...
fn resolve(typed: &str, candidates: &[NamedRef<'_>]) -> Option<EntityRef> {
    let lower = typed.to_lowercase();
    let rows: Vec<(EntityRef, String)> = candidates
        .iter()
...
mod tests {
    ...
    fn streams() -> Vec<NamedRef<'static>> {
        vec![
            NamedRef {
                id: sid(1),
                name: "Inbox",
    ...
    fn contexts() -> Vec<NamedRef<'static>> {
        vec![
            NamedRef {
                id: cid(1),
                name: "home",
    ...
    fn edit(input: &str) -> TaskEdit {
        parse(input, &streams(), &contexts(), now(), &TimeZone::UTC)
    }

    #[test]
    fn every_facet_the_patch_carries_is_reachable() {
        let e = edit("!2 %high ~90m ^tomorrow due:tomorrow @home #travel");
    ...
    fn unresolvable_tokens_are_reported_one_by_one() {
        let e = edit("#nope @nope !9 %sideways ~banana");
    ...
    #[test]
    fn a_row_the_caller_left_out_is_not_a_resolution_target() {
        // Archived rows are filtered by the caller, exactly as they are for
        // `capture::parse`; anything absent from the candidate list is
        // reported rather than guessed at.
        let ctxs = vec![contexts()[1]];
        let e = parse("@home", &streams(), &ctxs, now(), &TimeZone::UTC);
        assert_eq!(e.errors, vec![EditError::UnknownContext("home".into())]);
    }

    #[test]
    fn a_prefix_resolves_exactly_as_it_does_in_capture() {
        assert_eq!(edit("#trav").stream(), Some(sid(2)));
        assert_eq!(edit("@deep").patch_for(&[]).contexts, Some(vec![cid(2)]));
    }

    #[test]
    fn the_preview_says_what_will_change() {
        let line =
            edit("!1 %low @home #travel !-").preview(&streams(), &contexts(), &TimeZone::UTC);
