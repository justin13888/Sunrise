...
| `mise run macos-app` | **530 passed**, 0 failed; SwiftLint `--strict` clean; exit 0 |
| `mise run ios-app` | **551 passed**, 0 failed — 537 `SunriseiOSTests` + 14 `SunriseiOSUITests`; SwiftLint `--strict` clean; exit 0 |
...
them. The macOS 530 does **not** include the 5 XCUITest cases, which are
...
`apps/apple/SunriseTests/` declares **539** `@Test` cases. Nine of them are the
whole of `TabRouteTests.swift`, which is inside `#if os(iOS)` and describes a
shell macOS does not have: 539 − 9 = **530** on the Mac. Two of them are the
`#if os(macOS)` half of `MenuBarModelTests.swift`, which describes a menu bar
iOS does not have: 539 − 2 = **537** on the phone. Nothing else in the suite is
