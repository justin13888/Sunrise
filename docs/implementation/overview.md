...
proptest. The migration list is the `0013_baseline.sql` reset plus every append since,
so there is a multi-step upgrade chain to test as well as a refusal, and
`current_storage_v()` is asserted equal to `STORAGE_V`, so the constant and the
list cannot drift apart. Neither the count nor the value is repeated here: the
list is `crates/sunrise-storage/migrations/`, the constant is
`crates/sunrise-cbor/src/version.rs`, and
[`../04-storage/migrations.md`](../04-storage/migrations.md) is the page that
owns the subject. What is now tested against real old data rather than only
asserted is the chain itself — `crates/sunrise-storage/fixtures/` holds
committed vaults at two prior versions and the suite migrates them forward and
checks their contents survive.
...
| `mise run macos-app` | **622** cases in `SunriseTests` — 621 passed, 1 skipped, 0 failed; SwiftLint `--strict` clean; exit 0 |
| `mise run ios-app` | **638 passed**, 0 failed — 621 `SunriseiOSTests` + 17 `SunriseiOSUITests`; SwiftLint `--strict` clean; exit 0 |
...
them. The macOS 622 does **not** include the 5 XCUITest cases, which are
...
product as `SunriseiOSTests` and runs the 17 `SunriseiOSUITests` cases on the
simulator alongside them.

**The two Swift numbers differ, and the difference is exactly accounted for.**
`apps/apple/SunriseTests/` declares **631** `@Test` cases. Nine of them are the
whole of `TabRouteTests.swift`, which is inside `#if os(iOS)` and describes a
shell macOS does not have: 631 − 9 = **622** on the Mac. Ten are macOS-only, so
the phone drops those instead: the two `#if os(macOS)` cases that close
`MenuBarModelTests.swift`, which describe a menu bar iOS does not have, and the
**eight** of the `KeychainMigrationFallbackTests` suite, which sits whole inside
one `#if os(macOS) || targetEnvironment(macCatalyst)` at
`KeychainMigrationFallbackTests.swift:33-355`. So 631 − 10 = **621** on the
phone. Both sides close to the digit against what the two gates report.

Nothing else in the suite is platform-conditional **at the case level**, and
that is a claim about cases rather than about code. Three things trip up a
re-check, so they are written down here instead. `KeychainMigrationFallbackTests.swift`
declares **14** cases, not eight — the six below that `#endif`, in
`KeychainErrorMessageTests`, sit outside the conditional deliberately and the
doc comment at `:372` says why. `MenuBarModelTests.swift` declares **8**, of
which only the trailing two are conditional. And `grep -rl '#if os('` over the
directory returns **eleven** files while only ten hold a directive: the
eleventh hit is prose in a doc comment at
`KeychainMigrationVerifyTests.swift:12`. Of those ten, only the three files
named above wrap a `@Test` at all; in the other seven every `#if os(…)` closes
without enclosing one, so they move no count.

**How to re-derive all five figures** rather than adjusting them by hand:

* declared cases — `grep -rc '^[[:space:]]*@Test' apps/apple/SunriseTests/*.swift`
* suites — the distinct top-level types in those files that contain a `@Test`
* cases that ran — each job writes `out/test-results/{macos,ios}.xcresult`; ask
  it with `xcrun xcresulttool get test-results summary --path <bundle>`
* XCUITest cases — `grep -rcE '^[[:space:]]*func test' apps/apple/SunriseUITests/*.swift`,
  and the same over `apps/apple/SunriseiOSUITests/`
...
coverage is the black-box `crates/sunrise-cli/tests/cli.rs` suite driving `CARGO_BIN_EXE_sunrise`
