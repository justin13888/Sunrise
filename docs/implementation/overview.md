...
| `mise run macos-app` | **531 passed**, 0 failed; SwiftLint `--strict` clean; exit 0 |
| `mise run ios-app` | **552 passed**, 0 failed — 538 `SunriseiOSTests` + 14 `SunriseiOSUITests`; SwiftLint `--strict` clean; exit 0 |
...
them. The macOS 531 does **not** include the 5 XCUITest cases, which are
...
product as `SunriseiOSTests` and runs the 14 `SunriseiOSUITests` cases on the
simulator alongside them.

**The two Swift numbers differ, and the difference is exactly accounted for.**
`apps/apple/SunriseTests/` declares **540** `@Test` cases. Nine of them are the
whole of `TabRouteTests.swift`, which is inside `#if os(iOS)` and describes a
shell macOS does not have: 540 − 9 = **531** on the Mac. Two of them are the
`#if os(macOS)` half of `MenuBarModelTests.swift`, which describes a menu bar
iOS does not have: 540 − 2 = **538** on the phone. Nothing else in the suite is
platform-conditional at the case level.
