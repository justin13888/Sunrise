...
| Drag-and-drop | met | six of the eight cells, from shared files with no platform fork: `.draggable` on the row (`TaskRowView.swift:74`), drops on stream and context rows (`BrowseSidebar.swift:175`, `:218`), on task rows (`TaskListView.swift:192`), on the calendar grid (`CalendarView.swift:221`) and on the attachments pane (`AttachmentsView.swift:32`), plus `ForEach.onMove` for stream order (`BrowseSidebar.swift:45`). The gesture is a long-press drag rather than a click-drag. Two cells are **No**: *Calendar block → Task* is unbuilt on both platforms (`BlockChip` carries no `.draggable`; `TaskListView`'s drop only reorders), and *Task → Calendar block* — a **Yes** on macOS — fails on iOS for reach, both of its ends being shared and unguarded but never on screen together, with no second window available to fix that. Of the six, *File → Task* needs iPad multitasking ([`interaction-patterns.md`](interaction-patterns.md#drag-and-drop-matrix)) |
...
- **iOS.** Two SHOULDs are unmet, and both for the same shape of reason — a
  ...
  are shared and unguarded but the tab shell never shows them together, and the
  target declares no `UIApplicationSupportsMultipleScenes`, so there is no
  second window to span.
