...
| Drag-and-drop | met | the same seven of eight cells macOS has, from shared files with no platform fork: `.draggable` on the row (`TaskRowView.swift:74`), drops on stream and context rows (`BrowseSidebar.swift:175`, `:218`), on task rows (`TaskListView.swift:192`), on the calendar grid (`CalendarView.swift:221`) and on the attachments pane (`AttachmentsView.swift:32`), plus `ForEach.onMove` for stream order (`BrowseSidebar.swift:45`). The gesture is a long-press drag rather than a click-drag. The eighth cell (Calendar block → Task) is unbuilt on both platforms — `BlockChip` carries no `.draggable` and `TaskListView`'s drop only reorders — and two of the seven are qualified on iOS: *File → Task* needs iPad multitasking, and *Task → Calendar block* has both endpoints shared and unguarded but no screen that shows a task row and the grid at once ([`interaction-patterns.md`](interaction-patterns.md#drag-and-drop-matrix)) |
