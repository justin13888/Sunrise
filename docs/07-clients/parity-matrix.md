...
| Device list and revocation | met | `sunrise devices` marks revoked and non-current devices distinctly, and `sunrise device revoke <id-prefix> [--reason …]` rotates every Stream key — **and** the account identity, when run from the device that created the account ([ADR-0037](../11-adr/0037-identity-transition.md)). Only that device holds `ID_S_priv` since #105, so a revocation elsewhere cuts the keys and logs `core.identity.rotation_unavailable`; it costs little, because a revoked device that was itself paired cannot certify itself back in either way. It prints the two-state disclosure — revoked locally versus the relay has been told — because those are different guarantees and the second is the one a user pressing the button believes they are getting ([#160](https://github.com/justin13888/Sunrise/issues/160)). **Both Apple clients now have it too** ([#144](https://github.com/justin13888/Sunrise/issues/144)): `DeviceListSection` is a section of the Settings form that `VaultWindow` presents on macOS and `VaultTabs` on iOS, so one file serves both, and `DeviceListModel` drives it off `CoreQuery.deviceList` and the change feed. Removing a device is a swipe action and a context-menu item, confirmed through a dialog offering the four reasons the register records, and the result states both halves the CLI states plus the streams the rotation could not reach. Three signals reach a user there that previously reached none: this vault holding the only copy of `ID_D_priv`, a device id first seen after a revocation, and `CommandResult::unrotated_streams` |
| Identity status and rotation | met *(CLI only; no client surface)* | `sunrise identity status` prints the chain — the account's stable `genesis_identity_id`, the identity in force, the rotation count, whether this device still speaks for the account, whether it can seal a recovery code, and whether it can **add another device** — and `sunrise identity rotate [--new-recovery-code]` replaces the identity while keeping every current device, refusing by name on a device admitted by pairing. **Still CLI-only**, and narrower than the row above suggests: the Apple device list reads `SunriseCore::holds_identity_key` and `can_sponsor_pairing` directly, so the two facts a user acts on are on screen, but nothing in either app calls `Query::IdentityStatus` and neither offers a rotation |
...
| Capability | Verdict | Reached from |
|---|---|---|
| Read/write tasks | met | Today tab (`iOS/VaultTabs.swift:69-71`) → the shared `TaskListView` (`:153-170`); the row menu's **Edit…** (`TaskListView.swift:217`) opens the Mac's own `TaskEditorView`; swipe actions for Delete and Tomorrow (`:199-203`) |
| Streams, contexts, routines | met | Browse tab → `BrowseSidebar`: the toolbar's **Add** menu, which is where iOS puts what the Mac draws as a bottom bar (`:100-113`) → `StreamEditorView` / `ContextEditorView` (`:122-135`), and per-row Edit / Pause / Archive / Delete on a stream (`:268-285`) and on a context (`:311-318`). More → Routines → `RoutinesView`'s **New routine** (`:44`) |
| Today / Inbox / Stream views | met | Today is a tab root (`VaultTabs.swift:69-71`); the Inbox, a stream and a context push onto Browse instead, so the list they came from stays behind them (`iOS/TabRoute.swift:97-98`) |
| Focus mode | met | Focus tab (`VaultTabs.swift:87-93`) → the shared `FocusView`, and **Start focus session** on any row (`TaskListView.swift:225`) |
| Time-blocking on calendar grid | met | Calendar tab (`VaultTabs.swift:77-83`) → the shared `CalendarView`: drag-to-create (`CalendarView.swift:279`), block move (`:467`) and resize (`:504`), `BlockEditorView` on tap |
| Notes (rich text) | met | task editor → Notes pane → `NoteBodyEditor` (`TaskEditorView.swift:92`). Checklist ticks draw as filling circles rather than switches, because the iOS default for a `Toggle` says "this setting is on" where a checklist means "this is done" (`PlatformKit.swift:204-229`). Scope per ADR-0020 |
| Attachments — view image/PDF | met *(bytes this device can get)* | task editor → Attachments pane (`TaskEditorView.swift:93`); `PDFView` bridged through `UIViewRepresentable` (`AttachmentsView.swift:224-226`). Same scope note as the Mac's row and for the same reason: `apps/apple/Sunrise/` compiles into both targets, so the Download placeholder and its Cancel ([#227](https://github.com/justin13888/Sunrise/issues/227)) are literally the same view here. iOS carries one narrowing the Mac does not: `auto_fetch_on_cellular` is specified and unbuilt, so a phone on a cellular link fetches under-threshold attachments exactly as it would on Wi-Fi |
| Attachments — upload | met | `.fileImporter` (`AttachmentsView.swift:38`) and a URL drop target beside it (`:32`) — the same shared `AttachmentsModel` and the same core path the Mac reaches, since `apps/apple/Sunrise/` compiles into both targets (`project.yml:164`, `:322`), so the relay upload and the paired-device fetch are the Mac's ([#176](https://github.com/justin13888/Sunrise/issues/176)). See the macOS row above |
| Search (FTS) | met *(plain-text half)* | the `.search`-role tab (`VaultTabs.swift:97-99`) → the shared `SearchView`. It issues the identical literal-AND FTS query the Mac does and inherits the identical narrowness ([#28](https://github.com/justin13888/Sunrise/issues/28)) |
...
| Quick capture (system surface) | met | a **Capture** toolbar button on all five tab roots and on every pushed task list (`VaultTabs.swift:399-405`, attached at `:81`, `:91`, `:166`, `:177`, `:193`, `:282`; the six other pushed destinations at `:286-300` carry none, because a `.toolbar` on a `NavigationStack` root is not inherited by a `navigationDestination`), routed to the inline bar where the list has one and to the sheet where it does not (`:446-452`, `:455-464`); `sunrise://capture?text=`, registered by the iOS target in its own right (`project.yml:363-366`); and the **Capture Task** App Shortcut (`SunriseShortcuts.swift:23-33`) |
...
- **macOS.** No camera QR scanner exists — the *Pairing — scan QR* row's "camera
  ...
  skips two by decision. **Attachments now have a route to every byte**, and
  two pieces of the policy around it are still missing.
  [#176](https://github.com/justin13888/Sunrise/issues/176) built the automatic
  half and [#227](https://github.com/justin13888/Sunrise/issues/227) the asked-
  for half, so the greyed row is gone: under the threshold the pane waits a
  drain, over it there is a Download button
  (`apps/apple/Sunrise/Views/AttachmentsView.swift:99`) and a Cancel during
  transfer. What is left is **`auto_fetch_on_cellular`**, which
  [attachments.md](../02-domain/attachments.md) §Lazy fetch specifies as a
  per-platform setting and nothing implements — so a phone on a cellular link
...
- **iOS.** No SHOULD is unmet. The two that were — **saved views** and **iCal
  ...
  window to span either. The shell builds no path across the bar
  either — no `Tab` carries a `dropDestination` and nothing configures
  spring-loading, which `SunriseTests/TabDropTargetTests.swift` asserts over
  the `iOS/` sources on every build rather than leaving to a two-second
  simulator drag that could pass for the wrong reason
  ([#215](https://github.com/justin13888/Sunrise/issues/215)). What a held
  drag does on a bar nobody configured
  ([#72](https://github.com/justin13888/Sunrise/issues/72)) is not claimed
  here in either direction, and the two-handed gesture is beyond what
  XCUITest can express. Both halves are written up in
