...
> the twenty-seven `self.ops_insert` sites and the two `self.ops_insert_at`
> sites in `crates/sunrise-core/src/engine.rs`, the remote-apply path in the
> same file, and the storage crate's own tests. The loop therefore never runs
> and `op_dep` has **zero rows and zero readers**.
>
