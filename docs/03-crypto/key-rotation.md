...
* **§Revocation steps 1 and 2, at the key layer only.** `Command::RevokeDevice` emits a `device_revoke` op and rotates every stream in the rotation set — the vault-meta stream and the Inbox included, not only user Streams — sealing the new epochs to the devices that remain and not to the revoked one.

  **What revocation bounds here is key distribution, not writes.** `emit_key_envelopes` excludes a revoked device, so it receives no new epoch key and cannot read anything written after its cut; and a Stream key a revoked device *offers* is not absorbed, so it cannot make its peers seal under a key it holds. That property is the one the random-wrapped-key hierarchy exists to make expressible, and it was structurally impossible before this change — with `stream_key = BLAKE3(root‖stream‖epoch)` and `EPOCH = 1`, every device that ever knew the root could derive every key forever.

  **A revoked device's ordinary writes are still applied by its peers.** Peer-side refusal was in this change and was removed: a refused op leaves the refusing replica's sync cursor for that device frozen, the revoked device keeps its relay credentials so the relay's `max_seq` climbs past it, and within retention the eviction gap fires on every subscribe and latches `Degraded` — so an ordinary revocation would put every device in the account into a permanent data-loss warning, and no corrected cut could clear it. Bounding writes requires the relay to stop accepting them, which is [#80](https://github.com/justin13888/Sunrise/issues/80).

  The cut is the **HLC of the `device_revoke` op itself**; there is no `effective_at` field for an emitter to choose or for anyone to bound. Concurrent revocations resolve as an LWW register on that op's own `(hlc, device_id)` — the rule [ADR-0014](../11-adr/0014-full-state-ops.md) already resolves every other concurrent write in this engine with — so they converge regardless of arrival order *and* a later revocation supersedes an earlier one, which is what makes a cut that landed wrong correctable. A device cannot move its own cut.

  Because the HLC is chosen and signed by the sender, the cut is a **convergence** boundary and not a cryptographic one — see [`../01-architecture/threat-model.md`](../01-architecture/threat-model.md) §A3 for the residual and why this is nonetheless the right trade.
...
* **§Revocation steps 3 and 4.** The relay does not read `device_revoke` and does not refuse a revoked device's uploads — refusal is client-side, at `apply_remote`. Nothing wipes the revoked device's local database.
  ...
  **This costs churn, and the churn has no client-side fix.** A revoked device keeps its relay credentials, so it goes on uploading and the relay goes on offering those ops to every peer, which refuse them forever. The client cannot durably decide otherwise: advancing the sync cursor past a refusal is what makes the decision permanent, and the cut is a last-writer-wins register that can move either way, so a permanent decision is a wrong one. The fix belongs at the relay — `Command::RevokeDevice` calling `DELETE /api/v1/devices/{id}`, which already exists and already refuses a revoked device's signed uploads and sync sessions. That is not built: `sunrise-core` has no HTTP client, the relay API client lives in a crate only the CLI depends on, and `Engine::apply` is synchronous and inside a transaction — so it needs a durable intent the sync driver drains, with its own failure semantics for a revocation emitted while the relay is unreachable.
