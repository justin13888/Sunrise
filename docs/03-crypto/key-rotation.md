...
* **§Revocation steps 1 and 2.** `Command::RevokeDevice` emits a `device_revoke` op and rotates every stream in the rotation set — the vault-meta stream and the Inbox included, not only user Streams — sealing the new epochs to the devices that remain and not to the revoked one. Every replica that has applied the revocation refuses ops the revoked device signed at or after `effective_at`, and **applies the ones it signed before**: the cut is a time, compared against each envelope's own HLC in `Engine::is_revoked_at`. A replica that refuses an op records the refusal, so its sync cursor moves past it rather than stalling one seq short of it and having the relay replay it on every reconnect.
  Concurrent revocations converge on the **earliest** cut rather than the first to arrive, and a cut is refused unless it lies within `MAX_DRIFT_MS` behind and seven days ahead of the HLC of the op declaring it — which is what leaves the `now + 24h` overlap window in §Device key rotation usable while refusing a cut whose only purpose could be to immunise a device.
