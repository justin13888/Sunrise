...
* **A revoked device cannot claim a third device has already been sent a key.** The `key_envelope_recipients` row the `Recipient::Device(other)` arm files is what `backfill_key_envelopes` reads to decide a device is served, and filing one for somebody else is a way to withhold a key from them. A revoked sender's claim is no longer recorded. The table is a hint rather than state, so declining a row can only cause *more* key distribution and never less.
...
So the guarantee a user's device may promise is: **once this replica knows about the revocation, the revoked device reads nothing written afterwards, and it can neither revoke another device nor claim one has been keyed.** Not that it stops writing ordinary content, and not that what it wrote before the revocation reached everyone is rolled back. The decisions, their rejected alternatives and what would reopen them are [ADR-0034](../11-adr/0034-revocation-bounds-reads-not-writes.md) and [ADR-0041](../11-adr/0041-peer-side-revocation-is-a-fold.md).
