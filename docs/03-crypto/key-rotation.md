...
* **§Revocation steps 1 and 2.** `Command::RevokeDevice` emits a `device_revoke` op and rotates every stream in the rotation set — the vault-meta stream and the Inbox included, not only user Streams — sealing the new epochs to the devices that remain and not to the revoked one. Every replica that has applied the revocation refuses ops the revoked device signed at or after the cut, and **applies the ones it signed before**: the cut is a time, compared against each envelope's own HLC in `Engine::is_revoked_at`. A refusal records **nothing** — the op is not applied and the sync cursor does not move — so if a later revocation corrects the cut, the relay offers the op again and it applies. Nothing about a refusal is permanent, because the cut is not.

  **Revocation converges the cut, not the effect.** An op already applied when the revocation arrives is never re-examined, so two replicas holding identical op sets and an identical `devices` row can still hold different task tables: the one that saw the task op first applied it and kept it, the one that saw the revocation first refused it permanently. Which of those happened is a function of delivery order alone. That is a real limit of this design and not an implementation gap — closing it would mean retracting materialized state on a revocation, which nothing else in this engine does — and the cursor advance therefore records a delivery-order-dependent decision.
  The cut is the **HLC of the `device_revoke` op itself**; there is no `effective_at` field for an emitter to choose or for anyone to bound. Concurrent revocations resolve as an LWW register on that op's own `(hlc, device_id)` — the rule [ADR-0014](../11-adr/0014-full-state-ops.md) already resolves every other concurrent write in this engine with — so they converge regardless of arrival order *and* a later revocation supersedes an earlier one, which is what makes a cut that landed wrong correctable.
