...
* **§Revocation bounds a revoked device's reads and its writes, by two different mechanisms in two different places.** `Command::RevokeDevice` emits a `device_revoke` op, records the register, and rotates every stream in the rotation set — the vault-meta stream and the Inbox included.

  * *Reads* are bounded in the vault, and it takes two changes that are each vacuous alone. `emit_key_envelopes` anti-joins `device_revocations`, so a revoked device is sealed no envelope for any epoch minted at or after its cut; and `PairingPayload` no longer carries `ID_D_priv`, so there is no identity-sealed copy for it to open instead. While pairing handed every device the identity's unwrapping key, excluding a device from the recipient list withheld nothing — which is why an earlier slice removed the exclusion and filed [#76](https://github.com/justin13888/Sunrise/issues/76) rather than shipping half of it. Sealing needs only the public half, so the identity copy is still emitted for every epoch and recovery still reaches all of them.

    Removing that fallback opens a gap and `Engine::backfill_key_envelopes` closes it: a device certified *after* an epoch was minted was left out of that epoch permanently, and minting happens whenever a Stream is created. A replica applying a `device_cert` now seals what it holds to the newly certified device, and `key_envelope_recipients` (migration 0018) records who has been sent what so a backfill emits only what is missing and a re-published cert emits nothing. A revoked device is skipped, or revocation would be undone by re-sending a cert.

    **The exception, and it is a real one:** the device that *created* the account, and any device restored from the recovery code, hold `ID_D_priv` and can open the identity copy of any epoch. `Command::RevokeDevice` refuses to revoke the device it runs on, so this is reachable only by revoking the account's creator from another device. Until the recovery blob is built there is nowhere else for that key to live — dropping it would make an account with no surviving device unrecoverable — so the bound is stated rather than claimed.

  * *Writes* are bounded at the relay, not at the peer. `Command::RevokeDevice` queues a row in `relay_revocation_intents` (migration 0019) in the same transaction as the op, and the sync driver drains it once per session by calling `DELETE /api/v1/devices/{device_id}`; the relay then refuses the device's signed requests and ends any SSE session it is holding. It is a queue and not a call because revoking has to work offline — a device that is gone is the whole scenario. A refused call keeps the row and raises `attempts`, because a revocation that silently never arrived is the failure this exists to prevent.

    The relay cannot learn any of this from the op stream and must not be able to: `device_revoke` is sealed under the vault-meta Stream key, and promoting the revoked id into the cleartext envelope header would tell the relay which of an account's devices had been revoked and when, for every account it serves.
...
* **Identity rotation.** There is no `identity_transition` op, and two consequences follow from that rather than from the revocation design.
  ...
  The second is unchanged and is not closed by anything above: a revoked device still holds `ID_S_priv`, so it can issue itself a fresh valid `DeviceCert` under a new device id. Revocation names a device, and the identity keys are what name devices. What stands against it today is the relay, which will not accept a revoked device's upload of that cert — so the bound is the same one that bounds its writes, and it is a relay bound rather than a cryptographic one. Closing it properly needs step 1 of §Identity rotation.
