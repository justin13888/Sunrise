...
* **§Revocation steps 1 and 2.** `Command::RevokeDevice` emits a `device_revoke` op and rotates every stream in the rotation set — the vault-meta stream and the Inbox included, not only user Streams — sealing the new epochs to the devices that remain and not to the revoked one. Every replica that has applied the revocation refuses ops the revoked device signed at or after `effective_at`, and **applies the ones it signed before**: the cut is a time, compared against each envelope's own HLC in `Engine::is_revoked_at`. A replica that refuses an op advances its sync cursor over it, so the relay stops replaying it on every reconnect rather than the cursor stalling one seq short of it forever.
  ...
  **Revocation converges the cut, not the effect.** An op already applied when the revocation arrives is never re-examined, so two replicas holding identical op sets and an identical `devices` row can still hold different task tables: the one that saw the task op first applied it and kept it, the one that saw the revocation first refused it permanently. Which of those happened is a function of delivery order alone. That is a real limit of this design and not an implementation gap — closing it would mean retracting materialized state on a revocation, which nothing else in this engine does — and the cursor advance therefore records a delivery-order-dependent decision.
  ...
  Because the HLC is chosen and signed by the sender, `effective_at` is a **convergence** boundary and not a cryptographic one — see [`../01-architecture/threat-model.md`](../01-architecture/threat-model.md) §A3 for the residual and why this is nonetheless the right trade.
