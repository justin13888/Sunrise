...
**[ADR-0024](../11-adr/0024-key-hierarchy.md) is built, and most of this document is now a description rather than a target.** The two rows this ADR was about — identity-signed DeviceCerts and generated Stream keys — have closed and are gone from the table below; the four that remain are recorded so no reader mistakes a specification for a description. Everything outside the table stands as written.
...
| Public-key publication and the identity fingerprint | `POST /api/v1/accounts` requires and stores `identity_signing_pub` / `identity_dh_pub` (`crates/sunrise-server/src/api/accounts.rs` — an empty value for either is a `400 VALIDATION_INVALID`), but **no client sends a real key**. `AccountCreateRequest` has exactly one producer: `sunrise bootstrap`, whose `bootstrap_account` (`crates/sunrise-cli/src/main.rs`) fills both fields with the empty string, so the only request that reaches the route is one the route rejects. What changed with [ADR-0024](../11-adr/0024-key-hierarchy.md) is that there is now a key to send: `LocalKeychain` holds a real account identity keypair and persists it in the `identity` table, where before there was nothing to publish. There is still no identity-key-bundle lookup and no fingerprint UI. |

What has closed, and where to look:
