...
| Public-key publication and the identity fingerprint | `POST /api/v1/accounts` requires and stores `identity_signing_pub` / `identity_dh_pub` (`crates/sunrise-server/src/api/accounts.rs` — an empty value for either is a `400 VALIDATION_INVALID`), but **no client sends a real key**. `AccountCreateRequest` now has exactly one producer: `sunrise bootstrap`, whose `bootstrap_account` (`crates/sunrise-cli/src/main.rs`) fills both fields with the empty string, so the only request that reaches the route is one the route rejects. `IdentitySigningKeyPair::generate` / `IdentityDhKeyPair::generate` are still called only from `sunrise-crypto`'s own tests, so there is no key to send. There is no identity-key-bundle lookup and no fingerprint UI. |
