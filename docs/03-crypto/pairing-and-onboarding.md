...
- `account_email_hash` — `BLAKE3(email_normalized, 4)` rendered as 8 lowercase hex chars; identifies which account on the relay.
...
6. **On confirm, E sends transport messages to N** (each is a Noise transport message; the Noise tunnel is used as an authenticated, encrypted channel):
   ...
   ```
7. **N receives the payload, validates** that `BLAKE3("sunrise.identity_id.v1" || ID_S_pub, 16) == identity_id_bytes` (constant-time compare via `subtle::ConstantTimeEq`), generates its own `D_S` / `D_D` keypairs, and constructs a `DeviceCert` for itself signed by the just-received `ID_S_priv`.
