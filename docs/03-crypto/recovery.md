...
**The whole flow is now built on the CLI path, and partly built elsewhere.** Specifically:
...
* **`sunrise bootstrap` calls `seal_recovery_blob`.** It draws 32 bytes, seals the vault's identity keys under them, uploads the blob with `POST /api/v1/accounts`, and shows the 24-word code once — to stdout, never to a file and never to a log record. **The Apple clients do it too, as of #181.** `SunriseCore::bootstrap_account` is the same ceremony behind one FFI call — drawn, sealed, uploaded and dropped on the Rust side, so the code that reaches Swift is always the code the stored blob opens — and `RecoveryCodeModel` runs it the moment `SessionModel.createVault` returns, which is the moment the vault becomes the only place `ID_D_priv` exists. The app adds what a CLI cannot: the paste-back confirmation §Onboarding UX requirements has always specified. `Core::holds_identity_key` crosses the seam beside it, so the app can also *say* the condition rather than only fix it — and since #144 it does, in the Devices section of Settings, beside the list of what has access to the account.
* **`ID_D_priv` has a second copy wherever a blob has been sealed, which is now both clients.** Since `PairingPayload` stopped carrying the identity's X25519 secret, `Keychain::create` keeps it only on the device that *created* the account; every device admitted by pairing gets `dh_secret: None`. Where the blob has been sealed, that is the second copy and losing the creating device is survivable. Where it has not — a vault whose upload never completed, or one whose device has no relay configured — `ID_D_priv` is gone with that machine: every `Recipient::Identity` copy in the op log becomes permanently unopenable, and **no recovery feature added later can retrieve it**, because sealing a blob needs the key it would carry. `Keychain::holds_only_copy_of_identity_key` answers the question in the core API, `Core::holds_identity_key` passes it through, and the UniFFI seam now exports it, so both clients can ask.
...
* **`sunrise recover` spends the code.** `crates/sunrise-cli/src/recover.rs` is §Recovery flow steps 1 and 3 through 8 against a directory holding no vault: it signs the user in with `max_age=0` (the step-up the blob route demands, which a stored token cannot satisfy because its `auth_time` is whenever it happened), reads `ID_S_pub` off `GET /api/v1/accounts/me` and derives the `identity_id` the blob's AAD binds, fetches the blob, opens it with the typed words, mints a fresh vault root, opens the vault under `Unlock::RecoveryCode`, registers the re-keyed device, and waits for the replay before it reports success. That last clause is the point: reporting success before the log has been read hands the user an empty vault and calls it a recovery.
* **A recovered vault keeps `ID_D_priv`.** `Unlock::RecoveryCode` used to carry a `PairingPayload`, which since `#76` does not carry that key at all — so a vault seeded from one came up with `dh_secret: None` and could open no identity-addressed `key_envelope` whatsoever. `IdentitySeed::Recovered` carries the opened blob instead, and the recovering device records itself in `identity.minted_by_device_id` so the key survives the next open. `key-rotation.md` §Revocation already stated the rule this implements.
* **Recovery restores readable content, and one derived key is what makes that true.** The blob carries *identity* keys, and [ADR-0024](../11-adr/0024-key-hierarchy.md) seals every `(stream_id, epoch)` key to `ID_D_pub` as well as to each device — so a recovered identity opens the history. It could not, until the **vault-meta stream's genesis key stopped being random**. Every `key_envelope` op lives in the vault-meta stream and is sealed under that stream's key, including the ops that carry that key at epoch 1; so the identity's copy of the vault-meta genesis key was sealed under the vault-meta genesis key, and a recovering device holding `ID_D_priv` could open nothing with it. `Keychain::meta_genesis_key` derives that one key as `BLAKE3.derive_key("sunrise.meta_genesis_key.v1", ID_D_priv ‖ identity_id)`, which a recovering vault recomputes; every other `(stream, epoch)` key is still 32 random bytes. It adds no primitive outside [ADR-0004](../11-adr/0004-crypto-primitives.md)'s frozen suite, changes no stored or wire format, and costs nothing `ID_D_priv` did not already cost — see §The cost of that, stated plainly. What the blob does not carry is the **vault root**, which keys the local database: a recovering device mints its own. See §Recovery flow step 8, and [`identity-and-device-keys.md`](./identity-and-device-keys.md) §What is specified here vs. what is implemented.
* **The account record serves `ID_S_pub` back.** `GET /api/v1/accounts/me` carries `identity_signing_pub`, because a recovering client needs the vault's `identity_id` — the blob's AAD — *before* it can open the blob, and on a fresh device there is nowhere else to get it. `AccountInfo::identity_id` is the relay's own account id and never was this, so until that field existed the sentence "`expected_identity_id` MUST come from the OIDC account record" named a value the account record did not carry.
...
8. Stream keys themselves are NOT in the recovery blob. *(`sunrise recover` is the client that puts the pieces together; the replay needs no walker of its own, because each identity-addressed `key_envelope` arrives as an ordinary op and every absorbed key drains the ops parked in `deferred_ops` waiting for it. What had to be added was a way in: see §Implementation status on the vault-meta genesis key.)* They do not need to be. Per [ADR-0024](../11-adr/0024-key-hierarchy.md) decision 4, every `(stream_id, epoch)` key is sealed by HPKE to **two** recipient classes: to each device's `D_D_pub`, and to the **identity**'s `ID_D_pub`. The identity-sealed `key_envelope` ops live in the op log, which the relay stores as ciphertext it cannot open, and they are opened by `ID_D_priv` — which this blob already carries. The recovered device replays them and reads its history. **A pure recovery, with no surviving device and no peer, restores readable content.**

### The cost of that, stated plainly

This is not free, and the tradeoff belongs here rather than in a footnote. `ID_D_priv` becomes a **long-lived unwrapping key**: its compromise reaches every epoch ever sealed to it, and those envelopes sit on the relay where an attacker who has the key can also fetch them. Epoch rotation bounds *forward* exposure — a revoked device reads nothing written after the new epoch — and that bound exists only because a paired device is no longer handed `ID_D_priv` and so cannot open the identity's copy of a new epoch. The exception is the device that created the account, which still holds the key and therefore still opens every identity copy. **Revoking that device now rotates the identity away from it** ([ADR-0037](../11-adr/0037-identity-transition.md)): the successor is minted, every surviving device is re-certified under it, and the departing device holds no share — so epochs minted afterwards are sealed under an identity it cannot reach.

The cost lands on the recovery code, and the user has to be told. A rotation normally carries the successor forward by sealing `ID_S_priv || ID_D_priv` to the **outgoing** `ID_D_pub`, so the existing BIP-39 code keeps working. When the device being revoked is the one holding `ID_D_priv`, that carry is refused — it would seal the successor to the very key the revocation exists to exclude — and the old code stops opening the account. `core.identity.recovery_code_invalidated` fires, and `sunrise identity rotate --new-recovery-code` is the deliberate form of the same choice. What is *not* bounded, here or anywhere, is what the departing device already read: revocation is forward-only. Backward exposure was never bounded and nothing here pretends it was.
