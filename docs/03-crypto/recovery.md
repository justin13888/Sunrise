...
This is not free, and the tradeoff belongs here rather than in a footnote. `ID_D_priv` becomes a **long-lived unwrapping key**: its compromise reaches every epoch ever sealed to it, and those envelopes sit on the relay where an attacker who has the key can also fetch them. Epoch rotation was meant to bound *forward* exposure — a revoked device reading nothing written after the new epoch — and as built it does not, for exactly the reason this section is about: the revoked device was handed `ID_D_priv` when it was paired, so it opens the identity's copy of every new epoch too. Closing that needs identity rotation on revocation; see [`key-rotation.md`](./key-rotation.md) §Revocation. Backward exposure was never bounded and nothing here pretends it was.
