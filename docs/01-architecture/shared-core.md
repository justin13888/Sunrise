...
| rule | enforced by |
|---|---|
| 1 — clock | `clippy.toml`, for the `std::time` spellings. **Not** `tokio::time::Instant`, which `sync_driver` uses for backoff scheduling and which the path-based lint does not match — but that is a coverage hole, not a determinism one. `tokio::time::Instant` is **virtual under a paused runtime**, not an ambient wall clock, and `sunrise-core` already carries `tokio`'s `test-util` to drive it (`core.rs` uses `#[tokio::test(start_paused = true)]` with `tokio::time::advance`). **The harness cannot reach that capability.** `start_paused` works only on a `current_thread` runtime, and every `sync_driver` test that drives the reconnect loop is `#[tokio::test(flavor = "multi_thread")]` because its harness runs a fake server concurrently — so those deadlines are waited out on real time (a 400 ms sleep, three 10 s polling loops) rather than advanced. **The consequence is an untested branch:** `backoff_sleep`'s reset arm (`sync_driver.rs:592-593`), the flat un-jittered 30 s [`../05-sync/offline-queue.md`](../05-sync/offline-queue.md) documents as load-bearing, has no test at all, because testing it today means waiting thirty seconds. [#207](https://github.com/justin13888/Sunrise/issues/207) tracks fixing the harness. |
