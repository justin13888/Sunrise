...
**Residual: `effective_at` is a convergence boundary, not a cryptographic one.** The cut is a timestamp the revoked device *itself* chose and signed, and `Engine::is_revoked_at` compares it against exactly that. A revoked device that can still reach a peer can therefore date an op below the cut and have every replica apply it. Only the direction is constrained: the clock gate refuses an HLC reading in the receiver's future, so the lie can only ever run backwards.
...
What the implementation does guarantee is that the *revocation row* is not order-dependent. The cut is the HLC of the `device_revoke` op itself — there is no emitter-chosen `effective_at` — and concurrent revocations resolve as an LWW register on that op's own `(hlc, device_id)`. So every replica reaches the same cut whatever order it saw them in, and a cut that landed wrong is correctable by revoking again, which a converge-on-the-earliest rule was not.

**Residual: revocation converges the cut, not the effect.** An op already applied when the revocation arrives is never re-examined. Two replicas holding identical op sets and an identical `devices` row can therefore hold different task tables — the one that saw the task op first applied it and kept it, the one that saw the revocation first refused it permanently — and which happened is a function of delivery order alone. Closing that would mean retracting materialized state when a revocation lands, which nothing else in this engine does to anything. Until it is closed, "the revoked device's writes are bounded" is true of every replica individually and not of the account as a whole.
