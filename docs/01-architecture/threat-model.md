...
**Mitigations.**
- Device revocation: any other paired device can revoke; new per-Stream keys are issued; the revoked device stops being able to read what is written afterwards, and the relay stops accepting its uploads. **That much is enforced.** A revoked device is sealed no `key_envelope` for any epoch minted at or after its cut, and — since `PairingPayload` stopped carrying `ID_D_priv` — has no identity-sealed copy to open instead; the relay is told out of band (`DELETE /api/v1/devices/{id}`) and then refuses its signed requests and ends its stream. What is **not** enforced: the identity key does not rotate, so a revoked device keeps `ID_S_priv` and can issue itself a valid cert under a fresh device id (only the relay stands against it), and the account's *creator* keeps `ID_D_priv` until the recovery blob exists, so revoking that particular device does not bound its reads. Peers do not refuse a revoked device's ops, deliberately — see [`../03-crypto/key-rotation.md`](../03-crypto/key-rotation.md) §Revocation.

**Residual: the cut is a time, and it is a time the revoking device chose.** `emit_key_envelopes` compares `cut_ms` against the minting device's own wall clock, so a peer far enough ahead can write a cut that reads as "not yet revoked" and let one more epoch through. That direction is the safe one — it withholds nothing from a device that is still a member — and the rotation happens in the same transaction as the revocation, so the epoch a skewed cut lets through is superseded immediately. The clock gate bounds an incoming reading only from above, so this is a convergence boundary rather than a cryptographic one, and anyone tightening it should start there. A convergent peer-side check on *writes* is [#82](https://github.com/justin13888/Sunrise/issues/82).
...
**So: revocation is recorded, converged, and enforced — against a device that pairing admitted.** It stops that device reading anything written after its cut, and it stops the relay carrying its writes. It does not undo the past, it does not survive the device holding `ID_S_priv` if the relay is bypassed or self-hosted by the attacker, and it does not bind the account's creator. Read the two paragraphs above as the boundary of the claim: "revoke this device" is a real security action for the ordinary case and is not an absolute one, and the difference is identity rotation.
