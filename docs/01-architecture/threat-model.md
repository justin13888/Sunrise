...
**Mitigations.**
- Device revocation: any other paired device can revoke; identity key rotates; new per-Stream keys are issued; the revoked device's ops are no longer accepted by other devices. **Two of those three are implemented** ([ADR-0024](../11-adr/0024-key-hierarchy.md)): any paired device can revoke another, every stream the revoked device could read gets a fresh epoch sealed only to the devices that remain, and a revoked device stops being a recipient of new epoch keys — so it reads nothing written after its cut, and a key it offers is not absorbed by its peers. **The identity key does not rotate**, and until it does A3's denied goal is only half denied. Pairing hands each device `ID_D_priv`, and every epoch is sealed to the identity as well as to each device so that recovery can reach it, so a revoked device opens the identity's copy of every new epoch and keeps reading. It also keeps `ID_S_priv`, so it can issue itself a valid cert under a fresh device id. See [`../03-crypto/key-rotation.md`](../03-crypto/key-rotation.md) §Revocation. What is denied today is *writing*; what remains is *reading*.
...
**Residual: revocation does not bound the revoked device's writes at all.** Its peers apply the ops it keeps signing, exactly as they would from any member. Peer-side refusal was built and removed: a refused op freezes the refusing replica's sync cursor for that device, the revoked device keeps its relay credentials, and within the relay's retention window the eviction gap fires on every subscribe and latches a permanent data-loss warning on every device in the account — an ordinary administrative action producing an unrecoverable degraded state. Bounding writes needs the relay to stop accepting them ([#80](https://github.com/justin13888/Sunrise/issues/80)), not a client-side check.

What revocation does bound is **key distribution**, which is the property this hierarchy exists to make expressible: a revoked device receives no new epoch key, so it reads nothing written after its cut, and a key it offers is not absorbed by its peers, so it cannot make them seal under a key it holds. Closing that would mean retracting materialized state when a revocation lands, which nothing else in this engine does to anything. Until it is closed, "the revoked device's writes are bounded" is true of every replica individually and not of the account as a whole.

**So: revocation today is recorded, converged, and enforced at the key layer, and is not a security boundary.** It is how an account agrees that a device has left, and how that device stops being sent new keys. It is not a barrier that device cannot cross: it goes on writing until [#80](https://github.com/justin13888/Sunrise/issues/80) makes the relay refuse it; it reads through the rotation by way of the identity-sealed envelopes until [#76](https://github.com/justin13888/Sunrise/issues/76) rotates the identity; and the effect of a revocation still depends on delivery order until [#78](https://github.com/justin13888/Sunrise/issues/78) decides how to converge it. This document should not be read as describing complete revocation, and the PR that built it does not claim to.
