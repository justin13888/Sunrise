...
- The owner's device knows which Streams each recipient cohort can read (from `share_grant` records). Any reference to an entity in a Stream the cohort cannot read becomes `{kind: "redacted", reason: "private_ref", placeholder: "—"}` in the encrypted-for-cohort envelope. The owner retains the original (unscrubbed) form locally.
