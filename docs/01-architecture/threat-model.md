...
What the implementation does guarantee is that the *outcome* is not order-dependent. Concurrent revocations converge on the earliest cut (a `MIN` join over `(effective_at, revoker)`), and a cut is refused unless it sits close to the HLC of the op declaring it — so no single op can immunise a device with a cut no clock reaches, or reach backwards and invalidate everything it ever wrote.
