...
**Residual: revocation is forward-only, and two paths route around it.** It cannot take back what the revoked device already had — no rotation can, and the device keeps every key it held. Beyond that: the account's **creator** still holds `ID_D_priv`, because until the recovery blob is built there is nowhere else for it to live, so revoking that one device does not bound its reads. And any revoked device still holds `ID_S_priv` and can certify itself under a fresh device id: `self_authenticating_signer` admits the cert, applying it runs `backfill_key_envelopes`, and the new id is sealed every epoch the applying replica holds. **Nothing refuses that today**, and `sunrise_core`'s `a_revoked_device_rejoins_under_a_fresh_device_id` asserts the round trip rather than leaving it as a claim. The relay is sometimes named as what does. It is told about revocations now, and it still does not refuse this: the fresh device id arrives as a *fresh registration*, made with the OIDC bearer the device kept, so it is an unrevoked relay device row uploading its own certificate normally. **Refusing to backfill an id first seen in a cert from an already-revoked signer is also sometimes named, and it cannot be built: there is no signer to key it on.** A `DeviceCert` is signed by `ID_S_priv`, which is the account's rather than any device's, and the `device_cert` op is signed by the subject's own `D_S_priv`, which on a fresh id the revoked device minted too — so both signatures belong to the party producing the certificate and nothing attributes it to the revoked device. [ADR-0032](../11-adr/0032-revocation-cannot-bound-cert-issuance.md) prices the narrower shapes and records what kills each; identity rotation closes both residuals and is the only thing that does ([#105](https://github.com/justin13888/Sunrise/issues/105)). Peer-side refusal of a revoked device's ops is *not* the missing piece and is deliberately absent: refusing at apply time is not convergent without a projection rebuild, so a replica that applied an op before the revocation arrived would disagree with one that did not, permanently. [ADR-0034](../11-adr/0034-revocation-bounds-reads-not-writes.md) takes that decision and records why none of [#78](https://github.com/justin13888/Sunrise/issues/78)'s three options survived the refusal leaving the tree; [#82](https://github.com/justin13888/Sunrise/issues/82) is where a convergent form belongs, after the relay bound.
