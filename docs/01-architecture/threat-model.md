...
**Residual: revocation is forward-only, and two paths route around it.** It cannot take back what the revoked device already had — no rotation can, and the device keeps every key it held. Beyond that: the account's **creator** still holds `ID_D_priv`, because until the recovery blob is built there is nowhere else for it to live, so revoking that one device does not bound its reads. And any revoked device still holds `ID_S_priv` and can certify itself under a fresh device id: `self_authenticating_signer` admits the cert, applying it runs `backfill_key_envelopes`, and the new id is sealed the current epoch of every stream. **Nothing refuses that today.** The relay is sometimes named as what does; it is not told about revocations and cannot be ([#80](https://github.com/justin13888/Sunrise/issues/80)), so it refuses nothing. Identity rotation closes both of these; refusing to backfill an id first seen in a cert from an already-revoked signer would close the second one alone. Peer-side refusal of a revoked device's ops is *not* the missing piece and is deliberately absent: refusing at apply time is not convergent without a projection rebuild, so a replica that applied an op before the revocation arrived would disagree with one that did not, permanently ([ADR-0034](../11-adr/0034-revocation-bounds-reads-not-writes.md) takes that decision; [#82](https://github.com/justin13888/Sunrise/issues/82) is where a convergent form belongs, after the relay bound).
