...
This is honest disclosure to users, not a defect. The list is the relay's actual schema (`crates/sunrise-server/src/store/accounts.rs`, `store/devices.rs` and `relay_log.rs`), not a summary of it:
