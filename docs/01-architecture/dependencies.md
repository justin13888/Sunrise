...
|---|---|---|---|---|
| Rust toolchain | `rustc` (not a crate) | 1.91.1 | [ADR-0026](../11-adr/0026-msrv-bump.md) | Not a lockfile entry: `channel` in `rust-toolchain.toml` and `rust-version` in the root `Cargo.toml`, kept **byte-identical**, plus `ARG RUST_VERSION` in the `Dockerfile`. A reproducibility floor rather than a dependency workaround, raised from 1.88.0 to clear three things at once: `cargo-platform` 0.3.3 (≥ 1.91), `libsqlite3-sys` 0.38.1's `cfg_select!` (≥ 1.91, ADR-0012's blocker) and `std::fs::File::try_lock` (≥ 1.89, which retired `fs4`). CI's `msrv` job asserts the two literals against each other and against the `rustc` a checkout resolves. Raising it again requires an ADR. |
