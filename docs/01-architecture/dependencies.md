...
| HTTP server framework + OpenAPI document | `kynos` | 0.1.0 | [ADR-0021](../11-adr/0021-kynos-openapi-server.md) | `sunrise-server` only, `default-features = false` + `openapi32`/`macros`/`server`/`http1`/`http2`/`json`/`trace`. Routing, security schemes, middleware and the authoritative `schemas/openapi.v1.json` all come off the handlers. `openapi32` is load-bearing rather than a preference: 3.1 has no `itemSchema`, so `response::stream` is a 3.2-only subtree and [ADR-0023](../11-adr/0023-sse-sync-transport.md)'s event stream is undescribable without it. It replaced `axum`, which no longer resolves in `Cargo.lock`. |
