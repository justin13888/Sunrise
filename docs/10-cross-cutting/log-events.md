...
| `core.identity.transition_rejected` | warn | An `identity_transition` was not recorded as a link; `reason` (`id_derivation` / `roster` / `shares` / `roster_binding`), `sender_h`. Structural defects only — a chosen `to_identity_id` that is not the derivation of its own key, a roster entry that does not decode or names a device twice, a share of the wrong width, or a roster cert not issued by the successor identity. Signature failures are *not* here: `prev_sig` can only be checked against a predecessor this replica may not have established yet, so it is checked by the fold, where a transition that does not verify simply never becomes a link. Logged and dropped rather than failing the delivery, like every other rejection in `apply_control_op`. |
