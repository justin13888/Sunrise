...
| `sync.blob.uploaded` | info | An attachment's ciphertext is committed on the relay and readable by this account's other devices; `blob_h`, `chunk_count`. The moment the attachment stops being local-only. |
