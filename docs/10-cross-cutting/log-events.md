...
| `core.op.refused` | debug | An op this replica will never apply; `reason` (a closed set — see `RefusalReason`), `stream_h`, `device_h`, `seq`. The sync cursor is advanced over it, so the relay stops resending it. |
