...
/// Choosing an `.ics` to read, and somewhere to write one.
///
/// Split out of the menu items so the file half is testable without a panel:
/// ``read(_:)`` and ``write(_:to:)`` are the parts that can actually fail, and
/// `NSOpenPanel` is not something a test can drive.
enum IcalFiles {
    /// The types the open panel offers. `.ics` by extension rather than by a
    /// named `UTType` constant: the system's declaration for iCalendar is
    /// `com.apple.ical.ics`, which is Calendar.app's and not guaranteed to be
    /// registered on a Mac that has never opened one.
