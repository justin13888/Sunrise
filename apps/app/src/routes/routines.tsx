...
function RoutinesPage() {
    ...
    const stats = useMemo(() => {
        ...
        const totalTime = enabled.reduce((sum, r) => sum + r.duration.minutes, 0);
        const highPriority = enabled.filter(r => r.priority === 'high').length;
        ...
        <div className="max-w-7xl mx-auto p-6">
            ...
            <div className="mb-8">
                <div className="flex items-center justify-between mb-4">
                    <div>
                        <h1 className="text-3xl font-bold text-gray-900 mb-2">Routine Configuration</h1>
                        <p className="text-gray-600">Manage your daily routines and automation preferences</p>
                    </div>
                    <div className="flex items-center gap-3">
                        ...
                        <button type="button" className="px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-colors flex items-center gap-2">
                ...
                <div className="grid grid-cols-4 gap-4 mb-6">
                    <div className="bg-white rounded-xl p-4 border border-gray-200 shadow-sm">
                        <div className="flex items-center justify-between">
                            <div>
                                ...
                                <p className="text-2xl font-bold text-gray-900">{stats.totalRoutines}</p>
                            </div>
                    ...
                    <div className="bg-white rounded-xl p-4 border border-gray-200 shadow-sm">
                        <div className="flex items-center justify-between">
                            <div>
                                <p className="text-sm text-gray-600">Daily Time</p>
                                    ...
                                    <div className="divide-y divide-gray-100">
                                        {categoryRoutines.map((routine) => (
                                            <button
                                                key={routine.id}
                                                type="button"
                                                className={`w-full text-left p-4 hover:bg-gray-50 transition-colors cursor-pointer ${selectedRoutineId === routine.id ? 'bg-indigo-50 border-l-4 border-indigo-500' : ''
                                                ...
                                                <div className="flex items-center justify-between">
                                                    ...
                                                    <div className="flex items-center gap-2 ml-4">
                                                        <button
                                                            ...
                                                            onClick={(e) => {
                                                                e.stopPropagation();
                                                                toggleRoutineEnabled(routine.id);
                                                            }}
                                                            className="p-1 hover:bg-gray-200 rounded"
                                                        >
                                                            {routine.enabled ? (
                        ...
                        <div className="p-6">
                            <div className="flex items-center justify-between mb-4">
                                <h3 className="text-lg font-semibold text-gray-900">
                                    {editMode ? 'Edit Routine' : 'Routine Details'}
                                </h3>
                                <div className="flex items-center gap-2">
                                    {editMode ? (
                                        <>
                                            ...
                                            <button type="button" className="px-3 py-1 text-sm bg-indigo-600 text-white rounded hover:bg-indigo-700">
                                                Save
                                            </button>
                                        </>
                                    ) : (
                            ...
                            {editMode ? (
                                <div className="space-y-4">
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                        <div>
                                            ...
                                            <input
                                                id="duration"
                                                type="number"
                                                value={selectedRoutine.duration.minutes}
                                                className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                                            />
                                        </div>
                                        <div>
                                    ...
                                    <div>
                                        <label htmlFor="flexibility" className="block text-sm font-medium text-gray-700 mb-1">
                                            Flexibility ({selectedRoutine.flexibility}/10)
                                        </label>
                                        <input
                                            id="flexibility"
                                            type="range"
                                ...
                                <div className="space-y-6">
                                    <div>
                                        <h4 className="font-medium text-gray-900 mb-2">{selectedRoutine.name}</h4>
                                        <p className="text-sm text-gray-600 mb-4">{selectedRoutine.description}</p>

                                        <div className="grid grid-cols-2 gap-4 text-sm">
                                            <div>
                                                <span className="text-gray-500">Duration:</span>
                                                <span className="ml-2 font-medium">
                                                    {selectedRoutine.duration.flexible
                                            ...
                                            <div>
                                                ...
                                                <span className={`ml-2 px-2 py-1 text-xs rounded ${getPriorityColor(selectedRoutine.priority)}`}>
                                            ...
                                            <div>
                                                ...
                                                <span className="ml-2 flex items-center gap-1">
                                                    {getEnergyIcon(selectedRoutine.energy_level_required)}
                                    ...
                                    <div>
                                        <h5 className="font-medium text-gray-900 mb-2">Time Preferences</h5>
                                        <div className="flex flex-wrap gap-2">
                                            {selectedRoutine.time_preferences.map((time) => (
                                                <span key={time} className="px-2 py-1 bg-blue-100 text-blue-800 text-xs rounded-full">
                                                    {time.replace('_', ' ')}
                                                </span>
                                            ))}
                                        </div>
                                    ...
                                    <div>
                                        ...
                                        <div className="text-sm space-y-1">
                                            <div className="flex justify-between">
                                                ...
                                                <span>{selectedRoutine.flexibility}/10</span>
                                            </div>
                                            <div className="flex justify-between">
                                                <span className="text-gray-500">Buffer Time:</span>
                                                <span>{selectedRoutine.buffer_time_minutes} min</span>
                                            </div>
                                            <div className="flex justify-between">
                                                <span className="text-gray-500">Conflict Resolution:</span>
                                                <span className="capitalize">{selectedRoutine.conflict_resolution}</span>
                                            </div>
                                            <div className="flex justify-between">
                                                <span className="text-gray-500">Can Group:</span>
                                                <span>{selectedRoutine.can_be_grouped ? 'Yes' : 'No'}</span>
                                            </div>
                                        </div>
                                    </div>
